...
static int similar_enough(const char *cmd, unsigned int dist)
{
	...
	/*
	 * The similar subcommand finder doesn't account for prefix matched
	 * strings. Thus, fine-tuned levenshtein action weights (optimized for
	 * most cases) with a fixed threshold like 7 (which suits medium-length
